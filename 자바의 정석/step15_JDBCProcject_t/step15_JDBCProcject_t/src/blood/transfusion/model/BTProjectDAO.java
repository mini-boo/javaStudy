...
public class BTProjectDAO {
	...
	// 프로젝트 아이디로 프로젝트 내용 수정
		public static boolean updateBTProjectPurpose(String btProjectId, String btProjectContent) throws SQLException {
			Connection con = null;
	...
	public static boolean addBTProject(BTProjectDTO btProject) throws SQLException {
		...
		try {
			con = DBUtil.getConnection();
			pstmt = con.prepareStatement("insert bt_project values(?, ?, ?, ?, ?)");
	...
	// 프로젝트 아이디로 헌혈자 정보 수정
	public static boolean updateBTProjectdonor(String btProjectId, String btProjectContent) throws SQLException {
		Connection con = null;
		PreparedStatement pstmt = null;
		try {
			con = DBUtil.getConnection();

			pstmt = con.prepareStatement("update bt_project set donor_id=? where bt_project_Content=?");
			pstmt.setString(1, btProjectContent);
			pstmt.setString(2, btProjectId);
